...
datastory_name <- "sustainability_hackathon"
...
# characters etc.)
datastory_name_de <- "nutzung-snf-daten-visualisierungen-zur-nachhaltigkeit"
datastory_name_en <- "usage-snsf-data-a-sustainable-example"
datastory_name_fr <- "utilisation-donnees-fns-exemple-developpement-durable"

# English title and lead of the story (Mandatory, even if no EN version)
title_en <- "Using SNSF Data: A Sustainable Example" 
lead_en <- "The SNSF publishes a substantial amount of data about its research grants. This data is available to everyone, to discover trends in science, or dig deep into specific topics of interest, such as analysing sustainability research." 
# German title and lead of the story (Mandatory, even if no DE version)
title_de <- "Nutzung von SNF-Daten: Visualisierungen zum Thema Nachhaltigkeit" 
lead_de <- "Die Datensätze über unsere Forschungsförderung lassen sich vielseitig verwenden: zur Erforschung von Trends in der Wissenschaft ebenso wie für fundierte Themenanalysen – zum Beispiel über Nachhaltigkeit." 
# French title and lead of the story (Mandatory, even if no FR version)
title_fr <- "Utilisation des données du FNS : un exemple avec le développement durable"
lead_fr <- "Les données du FNS sont accessibles à tout le monde, que ce soit pour découvrir les tendances de la science ou pour explorer en profondeur des sujets spécifiques, comme par exemple la recherche sur le développement durable." 
# Contact persons, always (first name + last name)
contact_person <- c("Anne Jorstad")
...
publication_date <- "2022-03-28 02:00:00"
...
doi <- "https://doi.org/10.46446/datastory.sustainability-hackathon" 
# URL to Github page (optional)
github_url <- "" 
# Put Tag IDs here. Only choose already existing tags. 
tags_ids <- c(190, # sustainability
